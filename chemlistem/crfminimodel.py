...
class CRFMiniModel(object):
    ...
    def train(self, textfile, annotfile, runname, unsupfile, nunsup=0, unsupfile2=None):
        ...
        crf = CRF(len(self.lablist), learn_mode="join", test_mode="viterbi", sparse_target=False)
        crfl = crf(bl3d)
        model = Model(inputs=il, outputs=crfl )
        model.compile(loss=crf.loss_function, optimizer='rmsprop')
